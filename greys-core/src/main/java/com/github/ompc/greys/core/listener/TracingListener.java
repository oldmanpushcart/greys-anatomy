...
public abstract class TracingListener
        extends AdviceListener implements InvokeTraceable {

    // 修复问题 #78
    // 在当前类的<init>调用之前JVM会先调用super.<init>, 这些步骤只能被暂时跳过
    // 所以这里需要记录下被掉过的信息
    private final ThreadLocal<Stack<Tracing>> skipSuperInitStackRef = new ThreadLocal<Stack<Tracing>>() {
        @Override
        protected Stack<Tracing> initialValue() {
            return new Stack<Tracing>();
        }
    };
    ...
    private void popSuperInit() throws Throwable {
        ...
        final Stack<Tracing> stack = skipSuperInitStackRef.get();
        if (!stack.empty()) {
