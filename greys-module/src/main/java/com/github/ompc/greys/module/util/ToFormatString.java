 ...
 * 格式化输出
 */
public interface ToFormatString {
    ...
    class Factory {
        ...
        public static ToFormatString create(final Format format,
                                            final Object target) {
            return create(format, target, 2);
        }
        ...
        public static ToFormatString create(final Format format,
                                            ...
                                            final int expand) {
            if (null == format) {
                return create(Format.GA, target, expand);
            }
            switch (format) {
                case JSON:
                ...
                default:
                case GA:
                    return new ToGaString(target, expand);
            }
        }

    }

    class ToJsonString implements ToFormatString {

        private final static Gson gson = new Gson();
        private final Object target;

        public ToJsonString(final Object target) {
            this.target = target;
        }
     ...
     * 这坨代码真心不想动了...
     */
    class ToGaString implements ToFormatString {

        private final Object target;
        private final int expand;

        public ToGaString(final Object target, final int expand) {
            this.target = target;
            this.expand = expand;
        }
        ...
        @Override
        public String toFormatString() {
            return toString(target, 0, expand);
        }
